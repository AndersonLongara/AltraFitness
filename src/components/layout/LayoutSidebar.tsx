...
import { useSubscriptionNav } from "@/app/providers/SubscriptionNavProvider";

const allNavItems: Array<{ icon: typeof House; label: string; href: string; requiresSalesAccess?: boolean }> = [
    { icon: House, label: "Início", href: "/dashboard" },
    { icon: Funnel, label: "Vendas", href: "/dashboard/sales", requiresSalesAccess: true },
...
export default function LayoutSidebar() {
    ...
    const { hasSalesAccess } = useSubscriptionNav();
    const navItems = allNavItems.filter(
        (item) => !item.requiresSalesAccess || hasSalesAccess
    );
    ...
    return (
        <>
            {/* Desktop Sidebar */}
            <aside className="hidden md:flex flex-col w-24 h-screen bg-white dark:bg-[#1E2A36] fixed left-0 top-0 border-r border-slate-100 dark:border-white/10 z-50 py-8 items-center justify-between">
                {/* Logo */}
                <div className="w-12 h-12 bg-graphite-dark dark:bg-white/10 rounded-xl flex items-center justify-center text-performance-green dark:text-white font-extrabold text-xl">
                    A.
                </div>
                        ...
                        return (
                            <Link
                                ...
                                className={`p-2 rounded-2xl transition-all duration-300 group relative ${active
                                    ? "bg-performance-green text-white dark:text-graphite-dark shadow-lg shadow-emerald-200 dark:shadow-none"
                                    : "text-slate-400 hover:bg-slate-50 dark:hover:bg-white/10 hover:text-performance-green dark:hover:text-performance-green"
                                    }`}
                            >
                                <item.icon size={28} weight={active ? "fill" : "duotone"} />

                                {/* Tooltip */}
                                <span className="absolute left-14 bg-graphite-dark dark:bg-slate-700 text-white text-xs font-bold py-2 px-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">
                                    {item.label}
                                </span>
                            </Link>
                        );
            ...
            {/* Mobile Bottom Nav */}
            <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-pure-white/90 dark:bg-[#1a2332]/95 backdrop-blur-xl border-t border-slate-100 dark:border-white/10 safe-area-bottom">
