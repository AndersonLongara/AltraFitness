            ...
            <main className="max-w-4xl mx-auto p-6 md:p-8 space-y-6">
                ...
                <div className="bg-pure-white dark:bg-[#1E2A36] p-6 rounded-3xl soft-shadow space-y-6 border border-slate-100 dark:border-white/10">
                    <div className="relative">
                        <MagnifyingGlass size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
                        <input
                            type="text"
                            placeholder="Buscar aluno..."
                            className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-white/10 rounded-2xl font-medium text-slate-700 dark:text-slate-200 outline-none focus:ring-2 focus:ring-emerald-100 dark:focus:ring-emerald-500/30 transition-all border border-transparent dark:border-white/10 placeholder:text-slate-400 dark:placeholder:text-slate-500"
                            value={searchTerm}
                        ...
                        {filteredStudents.length > 0 ? (
                            filteredStudents.map(student => (
                                <button
                                    ...
                                    className="w-full flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-white/10 rounded-2xl transition-colors group text-left"
                                >
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-white/20 overflow-hidden shrink-0">
                                            {student.photoUrl ? (
                                                <img src={student.photoUrl} alt={student.name} className="w-full h-full object-cover" />
                                            ) : (
                                                <User size={24} weight="fill" className="text-slate-400 dark:text-slate-500 m-auto mt-3" />
                                            )}
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-slate-700 dark:text-slate-200 group-hover:text-graphite-dark dark:group-hover:text-white">{student.name}</h3>
                                            <p className="text-xs text-slate-400 dark:text-slate-500">{student.active ? 'Ativo' : 'Inativo'}</p>
                                        </div>
                                    </div>
                                    <div className="w-10 h-10 rounded-full bg-white dark:bg-white/10 flex items-center justify-center text-slate-300 dark:text-slate-500 group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-sm">
