            ...
            <div className="bg-white dark:bg-[#1E2A36] rounded-[32px] p-8 soft-shadow border border-slate-100 dark:border-white/10">
                <div className="flex items-start gap-6">
                    ...
                    <div className="relative">
                        <div className="w-24 h-24 rounded-2xl overflow-hidden bg-slate-100 dark:bg-white/10">
                            {profile.imageUrl ? (
                                <img
                                ...
                                <div className="w-full h-full bg-performance-green/10 dark:bg-performance-green/20 flex items-center justify-center">
                                    <span className="text-3xl font-black text-performance-green dark:text-emerald-400">
                                        {profile.name.charAt(0).toUpperCase()}
                                    </span>
                                </div>
                            )}
                        </div>
                        <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-performance-green rounded-full border-2 border-white dark:border-[#1E2A36]" />
                    </div>
                        ...
                        {isEditing ? (
                            <div className="flex items-center gap-3">
                                <input
                                    ...
                                    className="text-2xl font-extrabold text-graphite-dark dark:text-white bg-slate-50 dark:bg-white/10 border border-slate-200 dark:border-white/20 rounded-xl px-4 py-2 focus:outline-none focus:ring-2 focus:ring-performance-green/50 focus:border-performance-green"
                                    autoFocus
                                />
                                <button
                                    onClick={handleSave}
                                ...
                                <button
                                    ...
                                    className="p-2 bg-slate-100 dark:bg-white/10 text-slate-500 dark:text-slate-300 rounded-xl hover:bg-slate-200 dark:hover:bg-white/20 transition-colors"
                                >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    ...
                                    className="p-1.5 text-slate-400 hover:text-performance-green dark:hover:text-emerald-400 hover:bg-slate-50 dark:hover:bg-white/10 rounded-lg transition-all"
                                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-[#1E2A36] rounded-[24px] p-6 soft-shadow border border-slate-100 dark:border-white/10">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="text-xs font-semibold text-slate-400 dark:text-slate-500">Nome</label>
                            <p className="text-base font-bold text-graphite-dark dark:text-white">{profile.name}</p>
                        </div>
                        <div>
                            <label className="text-xs font-semibold text-slate-400 dark:text-slate-500">E-mail</label>
                            <p className="text-base font-bold text-graphite-dark dark:text-slate-200">{profile.email}</p>
                        </div>
                        {profile.cpf && (
                            <div>
                                <label className="text-xs font-semibold text-slate-400 dark:text-slate-500">CPF</label>
                                <p className="text-base font-bold text-graphite-dark dark:text-slate-200">{profile.cpf}</p>
                            </div>
                        )}
                        {profile.phone && (
                            <div className="flex items-center gap-2">
                                <Phone size={14} weight="duotone" className="text-slate-400 dark:text-slate-500" />
                                <div>
                                    <label className="text-xs font-semibold text-slate-400 dark:text-slate-500">Telefone</label>
                                    <p className="text-sm font-bold text-graphite-dark dark:text-slate-200">{profile.phone}</p>
                                </div>
                            </div>
                        )}
                        {profile.birthDate && (
                            <div>
                                <label className="text-xs font-semibold text-slate-400 dark:text-slate-500">Data de Nascimento</label>
                                <p className="text-sm font-bold text-graphite-dark dark:text-slate-200">
                                    {format(profile.birthDate, "dd/MM/yyyy")}
                                </p>
                            </div>
                        )}
                        <div>
                            <label className="text-xs font-semibold text-slate-400 dark:text-slate-500">ID do Trainer</label>
                            <p className="text-xs font-mono text-slate-400 dark:text-slate-500 mt-0.5">{profile.id}</p>
                        </div>
                    </div>
                </div>
                    ...
                    {/* Team Code */}
                        ...
                        <div className="bg-graphite-dark dark:bg-slate-200 rounded-[24px] p-6 soft-shadow">
                            <div className="flex items-center justify-between mb-3">
                                ...
                                <Users size={20} weight="duotone" className="text-slate-500 dark:text-slate-400" />
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex-1 bg-white/5 dark:bg-black/20 border border-white/10 rounded-xl px-4 py-3 text-center">
