...
export default function SettingsTabs({ activeTab, onTabChange }: SettingsTabsProps) {
    return (
        <div className="flex gap-2 bg-white dark:bg-[#1E2A36] rounded-2xl p-1.5 soft-shadow border border-slate-100 dark:border-white/10">
            {tabs.map((tab) => {
                ...
                return (
                    <button
                        ...
                        className={cn(
                            ...
                            isActive
                                ? "bg-performance-green text-white shadow-lg shadow-emerald-200 dark:shadow-none"
                                : "text-slate-400 hover:text-graphite-dark dark:hover:text-white hover:bg-slate-50 dark:hover:bg-white/10"
                        )}
                    >
