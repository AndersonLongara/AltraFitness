...
export default function PaymentsList({ payments, students, plans = [] }: { payments: Payment[]; students: Student[]; plans?: Plan[] }) {
    ...
    const [receivedPayment, setReceivedPayment] = useState({ studentId: "", amount: "", planId: "", paidAt: new Date().toISOString().slice(0, 10) });

    const handleCreate = async () => {
        setCreateError("");
        if (!newPayment.studentId || !newPayment.amount || !newPayment.dueDate) {
            setCreateError("Preencha aluno, valor e vencimento.");
            return;
        }
        const num = parseFloat(String(newPayment.amount).replace(",", "."));
        if (isNaN(num) || num <= 0 || num > 999999.99) {
            setCreateError("Valor inválido. Informe entre R$ 0,01 e R$ 999.999,99.");
            return;
        }
        ...
        try {
            ...
            setCreateError(e instanceof Error ? e.message : "Erro ao criar cobrança.");
        }
    };

    const handleRegisterReceived = async () => {
        setCreateError("");
        if (!receivedPayment.studentId || !receivedPayment.amount) {
            setCreateError("Preencha aluno e valor.");
            return;
        }
        const num = parseFloat(String(receivedPayment.amount).replace(",", "."));
        if (isNaN(num) || num <= 0 || num > 999999.99) {
            setCreateError("Valor inválido. Informe entre R$ 0,01 e R$ 999.999,99.");
            return;
        }
        try {
            await createPaymentReceived({
                studentId: receivedPayment.studentId,
                amount: Math.round(num * 100),
                paidAt: receivedPayment.paidAt ? new Date(receivedPayment.paidAt) : undefined,
                planId: receivedPayment.planId || undefined,
            });
            setIsRegisteringReceived(false);
            setReceivedPayment({ studentId: "", amount: "", planId: "", paidAt: new Date().toISOString().slice(0, 10) });
        } catch (e: unknown) {
            setCreateError(e instanceof Error ? e.message : "Erro ao registrar pagamento.");
        }
    };
    ...
    return (
        <div className="bg-white dark:bg-[#1E2A36] p-6 rounded-3xl soft-shadow border border-slate-100 dark:border-white/10 space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h3 className="text-xl font-bold text-graphite-dark dark:text-white">Histórico de Pagamentos</h3>
                    <p className="text-sm text-slate-400 dark:text-slate-500">Controle de mensalidades e cobranças.</p>
                </div>

                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    <button
                        onClick={() => { setIsCreating(true); setIsRegisteringReceived(false); setCreateError(""); }}
                    ...
                    <button
                        onClick={() => { setIsRegisteringReceived(true); setIsCreating(false); setCreateError(""); }}
                        className="p-4 bg-slate-100 dark:bg-white/10 text-graphite-dark dark:text-white rounded-xl hover:bg-slate-200 dark:hover:bg-white/20 transition-all font-bold flex items-center gap-2 text-sm"
                    >
            ...
            {createError && <p className="text-rose-600 dark:text-rose-400 text-sm font-medium mb-2">{createError}</p>}
            {isCreating && (
                <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-2xl animate-in slide-in-from-top-2 border border-slate-100 dark:border-white/10 mb-4">
            ...
            {isRegisteringReceived && (
                <div className="bg-emerald-50/50 dark:bg-emerald-900/20 p-4 rounded-2xl border border-emerald-100 dark:border-emerald-800/40 mb-4">
                    <p className="text-sm text-slate-600 dark:text-slate-300 font-medium mb-3">Registrar pagamento já recebido (PIX, dinheiro, transferência fora da plataforma). Opcional: defina o plano do aluno.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                        <select
                            ...
                            value={receivedPayment.studentId}
                            onChange={e => setReceivedPayment({ ...receivedPayment, studentId: e.target.value })}
                        >
                            <option value="">Selecione o Aluno</option>
                            {students?.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                        </select>
                        <div className="relative">
                            ...
                            <input
                                ...
                                value={receivedPayment.amount}
                                onChange={e => setReceivedPayment({ ...receivedPayment, amount: e.target.value })}
                            />
                        </div>
                        <select
                            className="p-2 rounded-xl border border-slate-200 dark:border-white/20 text-sm font-medium outline-none focus:border-performance-green bg-white dark:bg-white/5"
                            value={receivedPayment.planId}
                            onChange={e => setReceivedPayment({ ...receivedPayment, planId: e.target.value })}
                        >
                        ...
                        <input
                            ...
                            value={receivedPayment.paidAt}
                            onChange={e => setReceivedPayment({ ...receivedPayment, paidAt: e.target.value })}
                        />
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Se escolher um plano, o aluno terá esse plano vinculado e a data de vencimento será calculada automaticamente.</p>
                    <div className="flex justify-end gap-2">
                        <button
                            onClick={() => { setIsRegisteringReceived(false); setCreateError(""); }}
                            className="text-slate-500 dark:text-slate-400 text-xs font-bold px-4 py-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded-xl transition-colors"
                        >
                            Cancelar
                        </button>
            ...
            <div className="md:hidden space-y-2">
                {filteredPayments.length === 0 ? (
                    <p className="text-center py-8 text-slate-400 dark:text-slate-500 text-sm">Nenhum pagamento encontrado.</p>
                ) : (
                    filteredPayments.map(payment => {
                        ...
                        return (
                            <div key={payment.id} className="flex items-center gap-4 p-4 border-b border-slate-100 dark:border-white/10 last:border-0">
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-slate-700 dark:text-slate-200 text-sm truncate">{payment.student.name}</h4>
                                    <div className="flex items-center gap-2 mt-1 flex-wrap">
                                        ...
                                        <span className="text-xs text-slate-400 dark:text-slate-500">{format(payment.dueDate, "dd/MM/yyyy")}</span>
                                    </div>
                                </div>
                                <div className="text-right shrink-0">
                                    <span className="font-mono font-bold text-sm text-slate-700 dark:text-slate-200">{formatPrice(payment.amount)}</span>
                                    <div className="flex items-center justify-end gap-2 mt-1">
                                        {payment.status !== 'paid' && (
                                            <button
                                                onClick={() => handleMarkAsPaid(payment.id)}
                                                className="p-1 text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 rounded-lg transition-colors"
                                            >
                                                <CheckCircle size={16} />
                                            </button>
                                        )}
                                        <button
                                            ...
                                            className="p-1 text-slate-300 dark:text-slate-500 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-colors"
                                        >
                                            <Trash size={16} />
                                        </button>
                                    </div>
            ...
            <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-sm">
                        {filteredPayments.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="text-center py-8 text-slate-400 dark:text-slate-500">Nenhum pagamento encontrado.</td>
                            </tr>
                        ) : (
                            filteredPayments.map(payment => {
                                const isOverdue = payment.status === 'pending' && new Date(payment.dueDate) < new Date();
                                ...
                                return (
                                    <tr key={payment.id} className="group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors border-b border-slate-100 dark:border-white/10 last:border-0">
                                        <td className="py-4 pl-2 font-bold text-slate-700 dark:text-slate-200">{payment.student.name}</td>
                                        <td className="py-4 text-slate-500 dark:text-slate-400">{payment.plan?.name || '-'}</td>
                                        <td className="py-4 text-slate-500 dark:text-slate-400">
                                            {format(payment.dueDate, "dd/MM/yyyy")}
                                        </td>
                                        <td className="py-4 font-mono font-medium text-slate-600 dark:text-slate-300">
